package com.camunda.consulting.demo.camunda7demoapplication.externaltask;
...
@Component
@InternalTaskSubscription(topicName = "send-message")
public class SendMessageTaskHandler implements InternalTaskHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(SendMessageTaskHandler.class);

    @Override
    public void execute(InternalTask internalTask, InternalTaskService internalTaskService) {
        LOGGER.info("Handling external task {} on topic '{}' for process instance {}",
                internalTask.getId(), internalTask.getTopicName(), internalTask.getProcessInstanceId());
